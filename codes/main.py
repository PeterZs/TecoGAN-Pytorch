...
import yaml
from data import create_dataloader, prepare_data, prepare_data_face_degradation
...
def train(opt):
    ...
    # create downsampling kernels for BD utils
    ...
    for epoch in range(total_epoch):
        for data in train_loader:
            ...
            yaml_path = r'/home/SENSETIME/qianjinhao/program_project/TecoGAN-PyTorch-master/train_desktop.yaml'
            f = open(yaml_path, 'r')
            config = edict(yaml.load(f, Loader=yaml.FullLoader))

            Distort = Distortion_v2(config.pair_data.distortion, 1)
            data = prepare_data_face_degradation(opt, data, kernel, Distort)
